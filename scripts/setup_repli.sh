...
for port in $SLAVE1_PORT $SLAVE2_PORT; do
    ...
    MYSQL_PWD="$PASS" mariadb-dump -h 127.0.0.1 -P $MASTER_PORT -u$USER \
        --all-databases \
        --master-data=1 \
        --single-transaction \
        ...
        | MYSQL_PWD="$PASS" mariadb -h 127.0.0.1 -P $port -u$USER
    ...
    sleep 2
    STATUS=$(MYSQL_PWD="$PASS" mariadb -h 127.0.0.1 -P $port -u$USER -e "SHOW SLAVE STATUS\G")
