...
run_sql() {
    local port=$1
    local query=$2
    MYSQL_PWD="$PASS" mariadb $SSL_FLAGS -h 127.0.0.1 -P $port -u$USER -sN -e "$query" 2>/dev/null
}
...
for port in $SLAVE1_PORT $SLAVE2_PORT; do
    ...
    MYSQL_PWD="$PASS" mariadb-dump $SSL_FLAGS -h 127.0.0.1 -P $MASTER_PORT -u$USER \
        --all-databases \
        --master-data=1 \
        --single-transaction \
        ...
        | MYSQL_PWD="$PASS" mariadb $SSL_FLAGS -h 127.0.0.1 -P $port -u$USER
    ...
    sleep 2
    STATUS=$(MYSQL_PWD="$PASS" mariadb $SSL_FLAGS -h 127.0.0.1 -P $port -u$USER -e "SHOW SLAVE STATUS\G")
